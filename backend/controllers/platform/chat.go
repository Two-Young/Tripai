...
func CreateChatRoom(c *gin.Context) {
	...
	exists, err := platform.DoesChatRoomExistByParticipants(body.SessionId, body.Participants)
	if err != nil {
		log.Error(err)
		tx.Rollback()
		c.AbortWithStatus(http.StatusInternalServerError)
	...
	if exists {
		util2.AbortWithStrJson(c, http.StatusConflict, "chatroom already exists")
		return
	}
